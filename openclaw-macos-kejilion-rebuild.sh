...
openclaw_exists() { command -v openclaw >/dev/null 2>&1; }

ensure_macos() { [[ "$(uname -s)" == "Darwin" ]] || die "这个脚本只支持 macOS。"; }
...
ensure_xcode_clt() { xcode-select -p >/dev/null 2>&1 || { xcode-select --install || true; die "请先安装 Xcode Command Line Tools。"; }; }
ensure_homebrew() { command -v brew >/dev/null 2>&1 || /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"; load_brew_env; need_cmd brew; }
install_dependencies() { brew update; brew install git jq node python tmux coreutils gnu-tar sqlite || true; }
configure_npm_registry_if_needed() { local c=""; c=$(curl -fsSL --max-time 3 ipinfo.io/country 2>/dev/null || true); [[ "$c" == "CN" || "$c" == "HK" ]] && npm config set registry https://registry.npmmirror.com || true; }
ensure_openclaw_dirs() { mkdir -p "$OPENCLAW_HOME" "$WORKSPACE_DIR" "$BACKUP_DIR" "$OPENCLAW_HOME/logs" "$(dirname "$LAUNCH_AGENT_PLIST")"; }
ensure_openclaw_config() { ensure_openclaw_dirs; [[ -f "$OPENCLAW_CONFIG_FILE" ]] || printf '{}\n' > "$OPENCLAW_CONFIG_FILE"; }
...
install_openclaw(){ ensure_macos; ensure_xcode_clt; ensure_homebrew; install_dependencies; configure_npm_registry_if_needed; npm install -g "$OPENCLAW_NPM_PACKAGE"; need_cmd openclaw; ensure_openclaw_config; openclaw onboard || true; json_update_base; cecho "✅ 安装完成"; }
start_gateway(){ need_cmd openclaw; openclaw gateway stop >/dev/null 2>&1 || true; openclaw gateway start; cecho "✅ 已启动"; }
stop_gateway(){ need_cmd openclaw; openclaw gateway stop || true; cecho "✅ 已停止"; }
restart_gateway(){ need_cmd openclaw; openclaw gateway restart >/dev/null 2>&1 || { openclaw gateway stop >/dev/null 2>&1 || true; openclaw gateway start >/dev/null 2>&1; }; }
view_logs(){ need_cmd openclaw; openclaw status || true; echo; openclaw gateway status || true; echo; openclaw logs || true; }

change_model(){ need_cmd openclaw; cecho "当前模型列表："; openclaw models list || true; read -r -p "输入要切换的模型 ID（0 返回）：" m; [[ -z "$m" || "$m" == "0" ]] && return 0; openclaw models set "$m"; cecho "✅ 已切换到：$m"; }

provider_add_interactive(){
  need_cmd jq
  local provider_name base_url api_key models_json available_models default_model input_model confirm model_count=0
  read -r -p "请输入 Provider 名称: " provider_name; [[ -n "$provider_name" ]] || return 1
  read -r -p "请输入 Base URL (如 https://api.xxx.com/v1): " base_url; [[ -n "$base_url" ]] || return 1
  base_url="${base_url%/}"
  read -r -s -p "请输入 API Key: " api_key; echo; [[ -n "$api_key" ]] || return 1
  cecho "🔍 正在获取可用模型列表..."
  models_json=$(curl -s -m 10 -H "Authorization: Bearer $api_key" "${base_url}/models" || true)
  available_models=$(printf '%s' "$models_json" | python3 - <<'PY'
import sys,json
raw=sys.stdin.read().strip()
if not raw: raise SystemExit(0)
try:
 d=json.loads(raw)
 arr=d.get('data',[]) if isinstance(d,dict) else []
 ids=sorted(str(x.get('id')) for x in arr if isinstance(x,dict) and x.get('id'))
 print('\n'.join(ids))
except Exception:
 pass
PY
  ...
  if [[ -n "$available_models" ]]; then
    model_count=$(printf '%s\n' "$available_models" | sed '/^$/d' | wc -l | tr -d ' ')
    cecho "✅ 发现 $model_count 个模型："
    nl -w2 -s'. ' <<< "$available_models"
  fi
  read -r -p "请输入默认 Model ID 或序号（留空默认第一个）: " input_model
  if [[ -z "$input_model" && -n "$available_models" ]]; then default_model=$(printf '%s\n' "$available_models" | head -n1)
  elif [[ "$input_model" =~ ^[0-9]+$ ]]; then default_model=$(printf '%s\n' "$available_models" | sed -n "${input_model}p")
  else default_model="$input_model"; fi
  cecho "Provider: $provider_name"
  cecho "Base URL: $base_url"
  cecho "默认模型: $default_model"
  read -r -p "是否写入全部可用模型？(y/N): " confirm
  python3 - "$OPENCLAW_CONFIG_FILE" "$provider_name" "$base_url" "$api_key" "$default_model" "$available_models" "$confirm" <<'PY'
import json,sys
...
defs_models=defs.get('models')
if not isinstance(defs_models,dict): defs_models={}
defs['models']=defs_models
for m in models:
    defs_models.setdefault(f"{name}/{m['id']}",{})
json.dump(d,open(path,'w',encoding='utf-8'),ensure_ascii=False,indent=2); open(path,'a',encoding='utf-8').write('\n')
PY
  [[ -n "$default_model" ]] && openclaw models set "$provider_name/$default_model" || true
...
try:d=json.load(open(p,'r',encoding='utf-8'))
 ...
 print(f'❌ 读取配置失败: {e}'); raise SystemExit(0)
providers=((d.get('models') or {}).get('providers') or {})
if not providers:
 print('ℹ️ 当前未配置任何 API provider。'); raise SystemExit(0)
print('--- 已配置 API 列表 ---')
...
provider_sync(){
  read -r -p "请输入要同步的 provider 名称: " name; [[ -n "$name" ]] || return 1
...
provider_switch_protocol(){
  read -r -p "请输入 provider 名称: " name; [[ -n "$name" ]] || return 1
  cecho "1. openai-completions"
  cecho "2. openai-responses"
  read -r -p "请选择：" c
  local api=""; [[ "$c" == "1" ]] && api="openai-completions"; [[ "$c" == "2" ]] && api="openai-responses"; [[ -n "$api" ]] || return 1
  python3 - "$OPENCLAW_CONFIG_FILE" "$name" "$api" <<'PY'
import json,sys
...
provider_delete(){
  read -r -p "请输入要删除的 provider 名称: " name; [[ -n "$name" ]] || return 1
  python3 - "$OPENCLAW_CONFIG_FILE" "$name" <<'PY'
import json,sys
...
webui_add_origin(){ read -r -p "输入 origin：" origin; [[ -n "$origin" ]] || return 0; python3 - "$OPENCLAW_CONFIG_FILE" "$origin" <<'PY'
import json,sys
p,o=sys.argv[1:3]
d=json.load(open(p,'r',encoding='utf-8'))
...
PY
restart_gateway || true; }
webui_remove_origin(){ jq '.gateway.controlUi.allowedOrigins // []' "$OPENCLAW_CONFIG_FILE" 2>/dev/null || true; read -r -p "输入要移除的 origin：" origin; [[ -n "$origin" ]] || return 0; python3 - "$OPENCLAW_CONFIG_FILE" "$origin" <<'PY'
import json,sys
p,o=sys.argv[1:3]
d=json.load(open(p,'r',encoding='utf-8'))
...
memory_scheme_apply(){ local s="$1"; if [[ "$s" == "qmd" ]]; then openclaw config set memory.backend qmd || true; openclaw config set memory.qmd.command qmd || true; else openclaw config set memory.backend builtin || true; openclaw config set agents.defaults.memorySearch.provider local || true; fi; }
memory_setup_local(){ mkdir -p "$OPENCLAW_HOME/models/embedding"; local model="$OPENCLAW_HOME/models/embedding/embeddinggemma-300M-Q8_0.gguf"; if [[ ! -f "$model" ]]; then curl -L --fail --retry 2 -o "$model" "https://huggingface.co/ggml-org/embeddinggemma-300M-GGUF/resolve/main/embeddinggemma-300M-Q8_0.gguf" || warn "模型下载失败，你之后可手动补"; fi; openclaw config set agents.defaults.memorySearch.local.modelPath "$model" || true; }
memory_auto_setup(){ cecho "自动推荐逻辑：网络受限优先 QMD，否则优先 Local"; local scheme="local"; curl -I -m 2 -s https://huggingface.co >/dev/null 2>&1 || scheme="qmd"; cecho "推荐方案：$scheme"; read -r -p "输入 yes 确认部署：" y; [[ "$y" == "yes" ]] || return 0; if [[ "$scheme" == "qmd" ]]; then npm install -g @tobilu/qmd || true; memory_scheme_apply qmd; else memory_scheme_apply local; memory_setup_local; fi; openclaw memory index --force || true; restart_gateway || true; }
memory_fix_index(){ cecho "将执行全量重建索引"; read -r -p "输入 yes 确认：" y; [[ "$y" == "yes" ]] || return 0; openclaw memory index --force || true; restart_gateway || true; }
memory_view_files(){ mkdir -p "$WORKSPACE_DIR/memory"; find "$WORKSPACE_DIR" -maxdepth 2 \( -name 'MEMORY.md' -o -path '*/memory/*.md' \) -print | sort || true; }
...
memory_menu(){ while true; do clear; cecho "OpenClaw 记忆管理"; memory_status; echo; cecho "1. 更新记忆索引"; cecho "2. 查看记忆文件"; cecho "3. 索引修复"; cecho "4. 记忆方案（QMD/Local/Auto）"; cecho "0. 返回"; read -r -p "请选择：" c; case "$c" in 1) openclaw memory index || true; press_enter;; 2) memory_view_files; press_enter;; 3) memory_fix_index; press_enter;; 4) memory_scheme_menu;; 0) return 0;; *) warn "无效选项"; sleep 1;; esac; done; }

permission_backup_file(){ echo "$BACKUP_DIR/openclaw-permission-last.json"; }
permission_backup_current(){ cp -f "$OPENCLAW_CONFIG_FILE" "$(permission_backup_file)" 2>/dev/null || true; }
permission_restore_backup(){ [[ -f "$(permission_backup_file)" ]] && cp -f "$(permission_backup_file)" "$OPENCLAW_CONFIG_FILE" && restart_gateway; }
permission_render_status(){ python3 - "$OPENCLAW_CONFIG_FILE" <<'PY'
import json,sys
try:d=json.load(open(sys.argv[1],'r',encoding='utf-8'))
...
multiagent_menu(){ while true; do clear; cecho "多智能体管理"; multiagent_render_status; echo; cecho "1. 列出智能体"; cecho "2. 新增智能体"; cecho "3. 删除智能体"; cecho "4. 新增路由绑定"; cecho "5. 移除路由绑定"; cecho "6. 查看会话概况"; cecho "7. 健康检查"; cecho "0. 返回"; read -r -p "请选择：" c; case "$c" in 1) openclaw agents list || true; press_enter;; 2) read -r -p "输入 Agent ID：" aid; read -r -p "输入 workspace（默认 ~/.openclaw/workspace-$aid）：" ws; [[ -n "$aid" ]] && openclaw agents add "$aid" --workspace "${ws:-~/.openclaw/workspace-$aid}"; press_enter;; 3) read -r -p "输入 Agent ID：" aid; read -r -p "输入 DELETE 确认：" y; [[ "$y" == "DELETE" && -n "$aid" ]] && openclaw agents delete "$aid"; press_enter;; 4) read -r -p "输入 Agent ID：" aid; read -r -p "输入 bind 值：" bind; [[ -n "$aid" && -n "$bind" ]] && openclaw agents bind --agent "$aid" --bind "$bind"; press_enter;; 5) read -r -p "输入 Agent ID：" aid; read -r -p "输入 bind 值：" bind; [[ -n "$aid" && -n "$bind" ]] && openclaw agents unbind --agent "$aid" --bind "$bind"; press_enter;; 6) find "$OPENCLAW_HOME/agents" -name 'sessions.json' -maxdepth 3 2>/dev/null | sed 's#^#- #'; press_enter;; 7) openclaw config validate || true; find "$OPENCLAW_HOME/agents" -maxdepth 2 -type d 2>/dev/null | sed 's#^#- #'; press_enter;; 0) return 0;; *) warn "无效选项"; sleep 1;; esac; done; }

backup_project(){ local out="$BACKUP_DIR/openclaw-project-safe-$(date +%Y%m%d-%H%M%S).tar.gz"; local items=(); [[ -f "$OPENCLAW_HOME/openclaw.json" ]] && items+=("openclaw.json"); [[ -d "$OPENCLAW_HOME/workspace" ]] && items+=("workspace"); [[ -d "$OPENCLAW_HOME/extensions" ]] && items+=("extensions"); [[ -d "$OPENCLAW_HOME/skills" ]] && items+=("skills"); [[ -d "$OPENCLAW_HOME/prompts" ]] && items+=("prompts"); [[ -d "$OPENCLAW_HOME/tools" ]] && items+=("tools"); [[ ${#items[@]} -gt 0 ]] || die "没有可备份项目"; tar -czf "$out" -C "$OPENCLAW_HOME" "${items[@]}"; cecho "✅ 备份完成：$out"; }
backup_memory(){ local out="$BACKUP_DIR/openclaw-memory-full-$(date +%Y%m%d-%H%M%S).tar.gz"; local items=(); [[ -f "$WORKSPACE_DIR/MEMORY.md" ]] && items+=("MEMORY.md"); [[ -d "$WORKSPACE_DIR/memory" ]] && items+=("memory"); [[ ${#items[@]} -gt 0 ]] || die "没有可备份记忆文件"; tar -czf "$out" -C "$WORKSPACE_DIR" "${items[@]}"; cecho "✅ 备份完成：$out"; }
restore_backup(){ read -r -p "输入备份包完整路径：" fp; [[ -f "$fp" ]] || die "备份包不存在"; warn "这是高风险操作，会覆盖现有文件。"; read -r -p "确认继续输入 yes：" y; [[ "$y" == "yes" ]] || return 0; tar -xzf "$fp" -C "$OPENCLAW_HOME"; cecho "✅ 还原完成"; }
delete_backup(){ ls -1 "$BACKUP_DIR"/*.tar.gz 2>/dev/null || { warn "暂无备份文件"; return 0; }; read -r -p "输入要删除的备份完整路径：" fp; [[ -f "$fp" ]] || die "文件不存在"; rm -f "$fp"; cecho "✅ 已删除：$fp"; }
backup_restore_menu(){ while true; do clear; cecho "OpenClaw 备份与还原"; ls -1 "$BACKUP_DIR"/*.tar.gz 2>/dev/null || echo "暂无备份文件"; echo; cecho "1. 备份记忆全量"; cecho "2. 还原备份"; cecho "3. 备份 OpenClaw 项目"; cecho "4. 删除备份文件"; cecho "0. 返回"; read -r -p "请选择：" c; case "$c" in 1) backup_memory; press_enter;; 2) restore_backup; press_enter;; 3) backup_project; press_enter;; 4) delete_backup; press_enter;; 0) return 0;; *) warn "无效选项"; sleep 1;; esac; done; }

install_launch_agent(){ local oc_bin; oc_bin=$(command -v openclaw || true); [[ -n "$oc_bin" ]] || die "未找到 openclaw 命令"; cat > "$LAUNCH_AGENT_PLIST" <<EOF
...
remove_launch_agent(){ launchctl unload "$LAUNCH_AGENT_PLIST" >/dev/null 2>&1 || true; rm -f "$LAUNCH_AGENT_PLIST"; cecho "✅ 已移除开机自启"; }
update_openclaw(){ ensure_homebrew; install_dependencies; npm install -g "$OPENCLAW_NPM_PACKAGE"; restart_gateway || true; cecho "✅ 已更新 OpenClaw"; press_enter; }
uninstall_openclaw(){ read -r -p "确认卸载输入 yes：" y; [[ "$y" == "yes" ]] || return 0; remove_launch_agent || true; openclaw uninstall >/dev/null 2>&1 || true; npm uninstall -g openclaw || true; cecho "✅ 已卸载 npm 包，配置目录保留：$OPENCLAW_HOME"; press_enter; }

show_menu(){ clear; cecho "======================================="; cecho "🦞 OPENCLAW macOS 管理工具（复刻增强版）"; cecho "======================================="; cecho "1.  安装"; cecho "2.  启动"; cecho "3.  停止"; cecho "--------------------"; cecho "4.  状态日志查看"; cecho "5.  换模型"; cecho "6.  API管理"; cecho "7.  机器人连接对接"; cecho "8.  插件管理（安装/删除）"; cecho "9.  技能管理（安装/删除）"; cecho "10. 编辑主配置文件"; cecho "11. 配置向导"; cecho "12. 健康检测与修复"; cecho "13. WebUI访问与设置"; cecho "14. TUI命令行对话窗口"; cecho "15. 记忆/Memory"; cecho "16. 权限管理"; cecho "17. 多智能体管理"; cecho "--------------------"; cecho "18. 备份与还原"; cecho "19. 更新"; cecho "20. 卸载"; cecho "--------------------"; cecho "21. 安装开机自启（launchctl）"; cecho "22. 移除开机自启"; cecho "0. 退出"; cecho "--------------------"; }

main(){ ensure_macos; load_brew_env; ensure_openclaw_dirs; ensure_openclaw_config; while true; do show_menu; read -r -p "请输入选项并回车：" choice; case "$choice" in 1) install_openclaw; press_enter;; 2) start_gateway; press_enter;; 3) stop_gateway; press_enter;; 4) view_logs; press_enter;; 5) change_model; press_enter;; 6) api_manage_menu;; 7) bot_pairing_menu;; 8) plugin_manage;; 9) skill_manage;; 10) edit_main_config;; 11) run_onboard; press_enter;; 12) doctor_fix;; 13) webui_menu;; 14) tui_chat;; 15) memory_menu;; 16) permission_menu;; 17) multiagent_menu;; 18) backup_restore_menu;; 19) update_openclaw;; 20) uninstall_openclaw;; 21) install_launch_agent; press_enter;; 22) remove_launch_agent; press_enter;; 0) exit 0;; *) warn "无效选项"; sleep 1;; esac; done; }
main "$@"
